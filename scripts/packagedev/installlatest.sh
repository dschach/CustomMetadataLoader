...
VERSIONNUMBER=$(sf package version list --concise --packages "CustomMetadataLoader" | tail -1 | awk '{print $2}')

echo "Find latest PackageVersionId"
PACKAGEVERSIONID=$(sf package version list --concise --packages "CustomMetadataLoader" | tail -1 | awk '{print $3}')
...
if [ "$continue" = "y" ]; then
	...
	sf org create scratch --definition-file config/project-scratch-def.json --alias PackageInstallTest --no-namespace --duration-days 7 --no-ancestors --wait 20
	...
	sf project deploy start --manifest sfdx-source/unpackaged-managed/manifest.xml --target-org PackageInstallTest --wait 90 --post-destructive-changes sfdx-source/unpackaged-managed/destructiveChangesPost.xml --target-org PackageInstallTest
	sf project deploy start --source-dir sfdx-source/unpackaged --target-org PackageInstallTest --wait 90 --ignore-errors --ignore-warnings --target-org PackageInstallTest
	echo "Assigning permissions"
	sf org assign permset --name QuoteSyncTester --target-org PackageInstallTest

	#echo "Assigning permission set"
	#sf org assign permset --name QuoteSyncTester --target-org PackageInstallTest

	echo "Install sample data"
	sf apex run --file ./data/sample-data.apex --target-org PackageInstallTest
